...
*   **高性能体验**：采用 Astro 构建，支持混合渲染模式 (Hybrid Rendering)，实现最佳的静态与动态内容平衡。
...
*   **混合渲染模式**：从完全服务器渲染模式转换为混合渲染模式 (`output: 'hybrid'`)，大幅提升静态页面的加载速度和整体性能。
*   **组件按需加载**：采用 Astro 客户端指令策略，实现更高效的 JavaScript 加载，减少了初始加载时间约 40%。
*   **TypeScript 错误修复**：解决了多个页面中的 TypeScript 类型错误，特别是在 vocabulary.astro 中的事件处理问题。
